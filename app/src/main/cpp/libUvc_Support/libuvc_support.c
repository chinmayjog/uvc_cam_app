...
// Allocate a new camera structure for JNA
uvc_camera_t* allocate_camera_struct() {
    uvc_camera_t *camera = (uvc_camera_t*)malloc(sizeof(uvc_camera_t));
    if (camera != NULL) {
        memset(camera, 0, sizeof(uvc_camera_t));
        LOGD("Allocated camera struct at: %p", camera);
    } else {
        LOGE("Failed to allocate camera struct");
    }
    return camera;
}
